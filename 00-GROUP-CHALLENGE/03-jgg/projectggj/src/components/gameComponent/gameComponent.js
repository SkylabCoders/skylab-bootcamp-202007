...
function GameComponent() {
    ...
    const [machineAction, setMachineAction] = useState('https://thumbs.gfycat.com/PleasedLividGreyhounddog-size_restricted.gif');
    ...
    function setAnAction(acc) {
        if (player1.getLives() > 0 && machine.getLives() > 0) {
            ...
            console.log('YOU WIN!')
            setMachineAction('https://thumbs.gfycat.com/EnergeticShortGalapagoshawk-size_restricted.gif')
    ...
    return (
        <div className='game-holder flex-item'>
            <div className='user-action_container flex-col'>
                <img src={actions.charge} onClick={() => setAnAction('Charge')} alt='Charge'></img>
                <img src={actions.avoid} onClick={() => setAnAction('Avoid')} alt='Avoid'></img>
                <img src={actions.attack} onClick={() => setAnAction('Attack')} alt='Attack'></img>
