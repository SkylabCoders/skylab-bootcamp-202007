...
function airlines() {
	...
	console.log(
		'La media de precio de todos los vuelos es: ' +
			(acc / flights.length).toFixed(2) +
			'€'
	);
	...
	if (confirm('¿Eres ADMIN?')) {
		do {
			...
			var arr = prompt(
				'Introduce la información en el orden determinado1',
				'Barcelona,Menorca,200,false'
			).split(',');
			flights.push({
				id: flights.length,
				to: arr[0],
		...
		do {
			...
			var del = prompt(
				'Introduce el ID de los vuelos que quieres eliminar',
				'0,5,2'
			).split(',');
		...
		do {
			...
			switch (
				prompt(
					'Como quieres filtrar los precios? (a) Menos de X€ / (b) Más de X€ / (c) Igual a X€ / (d) Entre X€ y X€',
					'a'
				)
			) {
				case 'a':
					lim = parseInt(
						prompt('¿Que limite de precio quieres poner?', '2000')
					);
				...
				case 'b':
					lim = parseInt(
						prompt('¿A partir de que precio quieres buscar?', '2000')
					);
				...
				case 'd':
					result = entreAyA(
						flights,
						prompt('¿Que mínimo quieres poner?', '500'),
						prompt('¿I que máximo?', '1200')
					);
					break;
			}
			if (result.length != 0) {
