...
function showFlightInfo() {
	...
	for (let i = 0; i < flightInfo.length; i++) {
		...
		console.log(
			`The flight from ` +
				flightInfo[i].departure +
				` to ` +
				flightInfo[i].arrival +
				`, has a price of ` +
				flightInfo[i].price +
...
function getFlightsWithScales() {
	...
	return console.log(
		'Quantity of Flights with scales is: ' + flightsWithScales
	);
}
...
function getLast5FlightsArrivals() {
	let last5FlightsArrivals = flightInfo
		.slice(flightInfo.length - 5)
		.map((e) => e.arrival);
	return console.log(
		"Last 5 flight's arrivals are: " + last5FlightsArrivals.join(', ')
	);
}
...
function calculator(values) {
	...
	//If just one number value is passed through, we calculate its square and display the result
		...
		resultSet.push(
			`The square of ${values} is: ` + parseFloat(Math.sqrt(values).toFixed(3))
		);
	}
	...
	for (var i = 1; i > arguments.length; i++) {
		resultSet.push(
			'The sum of provided values is: ' +
				(values + Number(arguments[i].toFixed(3)))
		);
		resultSet.push(
			'The sub of provided values is: ' +
				(values - Number(arguments[i].toFixed(3)))
		);
		resultSet.push(
			'The multiply of provided values is: ' +
				values * Number(arguments[i].toFixed(3))
		);
		resultSet.push(
			'The division of provided values is: ' +
				values / Number(arguments[i].toFixed(3))
		);
	}
