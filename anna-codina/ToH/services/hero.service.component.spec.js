describe('HeroService', function () {
    ...
    it('should get hero list', function () {
        debugger;
        return heroService.getHeroList().then().then((list) => {
            expect(list).toEqual(heroList);
        });
    });
    it('should get one hero by id', function () {
        const id = 18;
        return heroService.getHeroById(id).then().then((actualHero) =>
    ...
    it('sholud throw error if not exist hero id', function () {
        ...
        return heroService.getHeroById(idError).then().catch((actualHero) =>
