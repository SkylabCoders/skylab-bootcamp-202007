let conectaQuatroGame = {
    ...
    drawPanel: function () {
        ...
        for (let i in this.panel) {
            ...
            let actualCircleInColumn = 0;
            for (let j in this.panel[i]) {
                if (this.panel[i][j] === 1) {
    ...
    checkVertical: function () {
        for (let i in this.panel) {
            if (this.panel[i].length >= this.conectToWin) {
                for (let j = 0; j < this.circlesByColumn; j++) {
    ...
    checkHoritzontal: function () {
        for (let i = 1; i <= this.totalColumn; i++) {
            let maxColumnIndex = false;
            for (let j = 0; j <= (this.circlesByColumn - 1); j++) {
                let actualColumn = "column" + String(i);
                let secondColumn = "column" + String(i + 1);
                let thirdColumn = "column" + String(i + 2);
                let forthColumn = "column" + String(i + 3);
    ...
    machineTurn: function () {
        let randomNumber = randomNumberGenerator(7);
        let matchColumn = false;
        do {
            randomNumber++;
            let actualColumn = "column" + String(randomNumber);
            if (this.panel[actualColumn].length < this.circlesByColumn) {
                this.panel[actualColumn].push(this.actualPlayer.index);
                matchColumn = true;
            } else {
...
function randomNumberGenerator(numberMax) {
    let actualRandomNumber = Math.random() * numberMax;
