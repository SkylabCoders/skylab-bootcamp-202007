...
function router(nav) {
	adminRoutes
		...
		.post((req, res) => {
			...
			const { title, type, description, img, price } = req.body;
			...
			(async function mongo() {
					try {
						...
						await collection.insertOne({ title, type, description, img, price });
		...
		.post((req, res) => {
			...
			(async function mongo() {
					try {
						...
						await collection.deleteOne({ _id: ObjectID(deleteProduct) });
