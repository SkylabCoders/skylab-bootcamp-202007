...
do{
    if(newFlight === true){
        ...
        var nuevaEscala = prompt('El vuelo posee escala : Y / N','Y / N').toUpperCase();
...
} while (confirm('Desea crear otro vuelo??'));
...
alert('Para comprar un billete, por favor introduzca en la consola: "comprar()" y siga las instrucciones.')
};
//Funcion a llamar para realizar la compra del vuelo.
function comprar(){
    let identificadorDeVuelo = prompt('Indique el ID del vuelo que desea comprar:','ej. 6');
    console.log('Usted ha comprado el vuelo ID '+identificadorDeVuelo+'. Gracias por su compra.')
};
