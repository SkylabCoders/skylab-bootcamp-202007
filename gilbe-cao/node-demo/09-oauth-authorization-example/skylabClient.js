...
var client = {
	client_id: 'skylab-client-1',
	client_secret: 'skylab-client-secret-1',
	redirect_uris: ['http://localhost:9000/callback'],
	scope: 'challenges membershipTime averageCodingLength'
};

var skylabPreCourseApi = 'http://localhost:9002/courseStats';
...
app.get('/courseStats', function (req, res) {
	if (!access_token) {
		if (refresh_token) {
	...
	var resource = request('GET', skylabPreCourseApi, { headers: headers });

	if (resource.statusCode >= 200 && resource.statusCode < 300) {
		var body = JSON.parse(resource.getBody());
		res.render('courseStats', { scope: scope, data: body });
		return;
	} else {
		access_token = null;
		if (refresh_token) {
